...
module.exports = {
  ...
  createUserRepeatingInterval: async (req, res, next) => {
    try {
      ...
      if(req.body.days == null || req.body.days.length < 1){
        const err = new Error ("days is a required field and it's length should be more than zero.")
        err.status = 400;
        return next(err);
      }
      helperFunctions.checkVarNotNull(req.body.title,"title is a required field.");
  ...
  updateUserSpecificInterval: async (req, res, next) => {
    try {
      ...
      helperFunctions.checkVarNotNull(intervalToUpdate,'Interval ID is not valid');
      currUser.save();
