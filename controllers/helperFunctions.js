...
module.exports = {
    getUser: async (token) => {
        ...
        if(currSession == null){
            const err = new Error("The authorization token is not valid. You should try logging in again.")
            err.status = 401
            throw (err)
        }
        ...
        if(currUser == null){
            const err =  new Error("The user account was not found. You should try logging in again.");
            err.status(403)
            throw (err);
        }
        return currUser
